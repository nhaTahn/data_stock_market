...
Improve the current VN model for reporting by keeping the base prediction model unchanged and improving the post-model selection layer:

```text
base LSTM prediction
-> filter signal / committee candidates
-> Wyckoff phase gate
-> transition pressure risk filter
-> holding-period execution with transaction cost
```

This is preferred over adding `market_leader_return` directly to the base LSTM because the market-leader experiments did not improve the main model robustly.
...
```text
The current improvement is not from adding another feature into the LSTM.
The stronger result comes from a post-model execution layer.

We keep the base LSTM prediction stream fixed, then use a filter/committee selector.
...
This suggests the current edge is in regime-aware selection and risk control, not in adding more raw sequence features.
```
