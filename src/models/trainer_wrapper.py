...
from src.models.dl_architectures.lstm_builder import (
...
def _primary_target_array(y: np.ndarray) -> np.ndarray:
...
def build_sign_magnitude_targets(
    ...
    primary_target = _primary_target_array(auxiliary_target if auxiliary_target is not None else signed_target)
...
def fit_sign_magnitude_model(
    ...
    history = model.fit(
        ...
        build_sign_magnitude_targets(
            y_train,
            auxiliary_target=_primary_target_array(y_train),
            use_log_magnitude=use_log_magnitude,
        ),
        ...
        validation_data=(
            ...
            build_sign_magnitude_targets(
                y_val,
                auxiliary_target=_primary_target_array(y_val),
                use_log_magnitude=use_log_magnitude,
            ),
