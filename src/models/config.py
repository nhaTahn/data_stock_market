...
MACRO_FEATURE_COLUMNS = ("market_leader_return", "vnindex_return", "a_d_ratio")
...
class LSTMConfig:
    ...
    lstm_units: int | list[int] = field(
        default_factory=lambda: (
            list(_cfg["hyperparameters"]["lstm_units"])
            if isinstance(_cfg["hyperparameters"]["lstm_units"], list)
            else _cfg["hyperparameters"]["lstm_units"]
        )
    )
    dropout: float = _cfg["hyperparameters"]["dropout"]
    recurrent_dropout: float = _cfg["hyperparameters"].get("recurrent_dropout", 0.0)
    use_layer_norm: bool = _cfg["hyperparameters"].get("use_layer_norm", False)
    ...
    market_leader_count: int = _cfg["hyperparameters"].get("market_leader_count", 10)
    market_leader_liquidity_window: int = _cfg["hyperparameters"].get("market_leader_liquidity_window", 60)
    market_leader_liquidity_min_periods: int = _cfg["hyperparameters"].get("market_leader_liquidity_min_periods", 20)
    use_stock_identity: bool = _cfg["hyperparameters"].get("use_stock_identity", True)
    ...
    feature_normalization_mode: str = _cfg["hyperparameters"].get("feature_normalization_mode", "none")
    feature_normalization_window: int = _cfg["hyperparameters"].get("feature_normalization_window", 60)
    feature_normalization_min_periods: int = _cfg["hyperparameters"].get("feature_normalization_min_periods", 20)
    feature_normalization_include_cs_z: bool = _cfg["hyperparameters"].get("feature_normalization_include_cs_z", True)
    feature_normalization_include_cs_rank: bool = _cfg["hyperparameters"].get("feature_normalization_include_cs_rank", True)
    feature_normalization_strict_past: bool = _cfg["hyperparameters"].get("feature_normalization_strict_past", True)
    feature_normalization_base_columns: tuple[str, ...] = ()
    feature_normalization_metadata: dict[str, object] = field(default_factory=dict)
    ...
    signmag_rank_loss_weight: float = _cfg["hyperparameters"].get("signmag_rank_loss_weight", 0.0)
    signmag_rank_temperature: float = _cfg["hyperparameters"].get("signmag_rank_temperature", 1.0)
    signmag_rank_min_group_size: int = _cfg["hyperparameters"].get("signmag_rank_min_group_size", 5)
    ...
    sample_weight_balance_mode: str = _cfg["hyperparameters"].get("sample_weight_balance_mode", "none")
    ...
    hetero_enabled: bool = _cfg["hyperparameters"].get("hetero_enabled", False)
    skip_enabled: bool = _cfg["hyperparameters"].get("skip_enabled", False)
    deep_head_enabled: bool = _cfg["hyperparameters"].get("deep_head_enabled", False)
    deep_head_hidden_units: int = _cfg["hyperparameters"].get("deep_head_hidden_units", 32)
    deep_head_dropout: float = _cfg["hyperparameters"].get("deep_head_dropout", 0.0)
    aux_plain_enabled: bool = _cfg["hyperparameters"].get("aux_plain_enabled", False)
    aux_plain_pred_loss_weight: float = _cfg["hyperparameters"].get("aux_plain_pred_loss_weight", 1.0)
    aux_plain_sign_loss_weight: float = _cfg["hyperparameters"].get("aux_plain_sign_loss_weight", 0.25)
    aux_plain_magnitude_loss_weight: float = _cfg["hyperparameters"].get("aux_plain_magnitude_loss_weight", 0.25)
    aux_plain_log_magnitude: bool = _cfg["hyperparameters"].get("aux_plain_log_magnitude", True)
    ...
    pcie_lite_context_columns: tuple[str, ...] = tuple(
        _cfg["hyperparameters"].get("pcie_lite_context_columns", ["vnindex_return", "market_leader_return", "a_d_ratio", "day_of_week"])
    )
    ...
    regime_filter: str | None = _cfg["hyperparameters"].get("regime_filter")
...
def get_config(target_mode: str = "return", market: str | None = None) -> LSTMConfig:
