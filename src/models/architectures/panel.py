...
def build_panel_model(
    ...
    sequence_input, encoded = build_lstm_backbone(
        window_size=window_size,
        num_features=num_features,
        lstm_units=lstm_units,
        dropout=dropout,
    )
    stock_input = layers.Input(shape=(1,), dtype="int32", name="stock_id")
    stock_embedding = layers.Embedding(
        input_dim=num_stocks,
        output_dim=embedding_dim,
        name="stock_embedding",
    )(stock_input)
    stock_embedding = layers.Flatten(name="stock_embedding_flat")(stock_embedding)
    stock_context = layers.Dense(head_units, activation="relu", name="stock_context")(stock_embedding)
    merged = layers.Concatenate(name="panel_concat")([encoded, stock_context])
    merged = layers.Dense(head_units, activation="relu", name="panel_hidden")(merged)
    output = layers.Dense(1, name="panel_prediction")(merged)
    model = keras.Model(inputs=[sequence_input, stock_input], outputs=output)
    model.compile(
