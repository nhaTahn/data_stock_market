...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run narrow signal-paper phase 2 experiments.")
    parser.add_argument("--stamp", default="20260417_signal_phase2")
    ...
    parser.add_argument("--signal-future-steps", default="1,3,5")
    parser.add_argument("--signal-patch-length", type=int, default=5)
    parser.add_argument("--signal-patch-stride", type=int, default=3)
    parser.add_argument("--signal-patch-dim", type=int, default=16)
    parser.add_argument("--signal-attention-heads", type=int, default=2)
    parser.add_argument("--signal-attention-key-dim", type=int, default=16)
    parser.add_argument("--signal-attention-ff-dim", type=int, default=64)
...
def summarize_run(run_dir: Path) -> dict[str, object]:
    ...
    summary: dict[str, object] = {
        ...
        "feature_phase": config.get("feature_phase"),
        "sequence_normalization": config.get("sequence_normalization"),
        "signal_future_steps": config.get("lstm_signal_future_steps"),
        "signal_patch_length": config.get("lstm_signal_patch_length"),
        "signal_patch_stride": config.get("lstm_signal_patch_stride"),
        "signal_patch_dim": config.get("lstm_signal_patch_dim"),
    }
    ...
    signal_best_test = summary.get("lstm_signal_best_test_rel_score")
    lstm_best_test = summary.get("lstm_best_test_rel_score")
    if signal_best_test is not None and lstm_best_test is not None:
        summary["signal_gain_vs_lstm_best_test"] = float(signal_best_test) - float(lstm_best_test)
    else:
        summary["signal_gain_vs_lstm_best_test"] = None
    return summary


def build_cases(args: argparse.Namespace) -> list[tuple[str, UniversePreset, int]]:
    selected_universes = args.include_universe or ["bds", "bank", "vin"]
    future_steps = parse_csv_ints(args.signal_future_steps)
    cases: list[tuple[str, UniversePreset, int]] = []
    for universe_name in selected_universes:
        preset = UNIVERSE_PRESETS[universe_name]
        for steps in future_steps:
            case_name = f"{preset.name}_h{steps}"
            cases.append((case_name, preset, steps))
    if args.only_case:
...
def build_cmd(args: argparse.Namespace, run_name: str, preset: UniversePreset, future_steps: int) -> list[str]:
    cmd = [
        str(PYTHON_BIN),
        str(MAIN_CLI),
        "train",
        ...
        "--enable-signal-family",
        "--signal-future-steps",
        str(future_steps),
        "--signal-patch-length",
        str(args.signal_patch_length),
        "--signal-patch-stride",
        str(args.signal_patch_stride),
        "--signal-patch-dim",
        str(args.signal_patch_dim),
        "--signal-attention-heads",
        str(args.signal_attention_heads),
        "--signal-attention-key-dim",
        str(args.signal_attention_key_dim),
        "--signal-attention-ff-dim",
        str(args.signal_attention_ff_dim),
    ]
...
def main() -> None:
    args = parse_args()
    log_dir = LOG_BASE / f"{args.stamp}_signal_phase2"
    log_dir.mkdir(parents=True, exist_ok=True)

    rows: list[dict[str, object]] = []
    for case_name, preset, future_steps in build_cases(args):
        run_name = f"{case_name}_{args.stamp}"
        log_path = log_dir / f"{run_name}.log"
        cmd = build_cmd(args, run_name, preset, future_steps)
        ...
        row: dict[str, object] = {
            ...
            "signal_future_steps": future_steps,
    ...
    summary = {
        "summary_path": str(log_dir / "signal_phase2_summary.json"),
        "cases": rows,
    }
    (log_dir / "signal_phase2_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
